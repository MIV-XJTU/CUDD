...
sys.path.append('..')
# It is imported for you to access and modify the PyTorch source code (via Ctrl+Click), more details in SRe2L's README.md
...
def main():
    ...
    if args.student_dir is not None or args.model == 'swin_t':
        args.adamw_lr = 5e-4
    if args.model == 'deit_tiny':
        model = timm.create_model('deit_tiny_patch16_224', num_classes=1000, pretrained=False)
    elif args.model == 'mixer_s':
        model = timm.create_model('mixer_s16_224', num_classes=1000, pretrained=False)
    elif args.model == 'resmlp_24':
        model = timm.create_model('resmlp_24_224', num_classes=1000, pretrained=False)
    else:
...
def save_checkpoint(state, is_best, output_dir=None,epoch=None):
    if epoch is None:
        ...
        path = output_dir + f'/checkpoint_{epoch}.pth'
    torch.save(state, path)

    if is_best:
        path_best = output_dir + '/' + 'model_best.pth'
